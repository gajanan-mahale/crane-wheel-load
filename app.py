...
st.set_page_config(page_title="Single Girder Wheel Load calc", layout="wide")
st.title("Single Girder Crane - Wheel Load Calc for M5 class")

try:
    df = pd.read_excel("crane_data.xlsx", sheet_name="SG TABLE")
    # Keep only first 4 columns and clean
    ...
    # FIND girder & crane weight from Excel
    ...
    # Assumptions - you can change in UI
    # impact = 1.25
    crab_kg = swl_input * 0.1 # estimate
    wheels = 4
    ...
    k2.metric("Crane wt (with girder)", f"{crane_kg:.0f} kg")
    k3.metric("Crab wt (est)", f"{crab_kg:.0f} kg")
    k4.metric("Live_Load", f"{Live_Load:.0f} kg")

    st.success(f"**MAX LT Wheel = {max_wheel:.0f} kg**")
    st.info(f"**MIN LT Wheel = {min_wheel:.0f} kg**")
    st.write(f"Max End Reaction {R_max_end:.0f} kg | Min End Reaction {R_min_end:.0f} kg")

except FileNotFoundError:
    st.error("crane_data.xlsx not found. Put it in same folder as app.py")